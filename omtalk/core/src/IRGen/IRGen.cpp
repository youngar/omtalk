...
public:
  ...
  omtalk::ConstantIntOp irGen(const IntegerExpr &expr) {
    auto location = loc(expr.location);
    auto value = builder.getIntegerAttr(builder.getIntegerType(64), expr.value);
    return builder.create<omtalk::ConstantIntOp>(
        location, omtalk::BoxIntType::get(builder.getContext()), value);
  }

  omtalk::ConstantFloatOp irGen(const FloatExpr &expr) {
    auto location = loc(expr.location);
    auto value = builder.getFloatAttr(builder.getF64Type(), expr.value);
    return builder.create<omtalk::ConstantFloatOp>(
        location, omtalk::BoxRefType::get(builder.getContext()), value);
  }

  omtalk::ConstantStringOp irGen(const StringExpr &expr) {
    auto location = loc(expr.location);
    auto value = builder.getStringAttr(expr.value);
    return builder.create<omtalk::ConstantStringOp>(
        location, omtalk::BoxRefType::get(builder.getContext()), value);
  }

  omtalk::ConstantSymbolOp irGen(const SymbolExpr &expr) {
    auto location = loc(expr.location);
    auto value = builder.getStringAttr(expr.value);
    return builder.create<omtalk::ConstantSymbolOp>(
        location, omtalk::BoxRefType::get(builder.getContext()), value);
  }
  ...
  mlir::Value irGenNil(const IdentifierExpr &expr) {
    ...
    return builder.create<omtalk::ConstantRefOp>(
        location, omtalk::BoxRefType::get(builder.getContext()), value);
  }
  ...
  omtalk::SendOp irGen(const SendExpr &expr) {
    auto ty = omtalk::BoxUnkType::get(builder.getContext());
  ...
  omtalk::ReturnOp irGen(const ReturnExpr &expr) {
    auto value = irGenExpr(*expr.value);
    auto location = loc(expr.location);
    return builder.create<omtalk::ReturnOp>(location, value);
  }

  omtalk::NonlocalReturnOp irGen(const NonlocalReturnExpr &expr) {
    auto value = irGenExpr(*expr.value);
    auto location = loc(expr.location);
    return builder.create<omtalk::NonlocalReturnOp>(location, value);
  }
  ...
  mlir::Value irGen(const SelfExpr &expr) { return symbolTable.lookup("self"); }
  ...
  omtalk::BlockOp irGen(const BlockExpr &expr) {
    auto ty = omtalk::BoxUnkType::get(builder.getContext());
    auto location = loc(expr.location);
    ...
    auto blockOp = builder.create<omtalk::BlockOp>(
  ...
  omtalk::MethodOp irGen(const Method &method) {
    auto location = loc(method.location);
    auto sym_name = getSelectorString(method.selector);
    auto methodOp = builder.create<omtalk::MethodOp>(
  ...
  omtalk::KlassOp irGen(const Klass &klass) {
    ...
    auto klassOp = builder.create<omtalk::KlassOp>(location, name, super);
    ...
    if (klass.fields) {
      for (const auto &field : (*klass.fields).elements) {
        builder.create<omtalk::FieldOp>(loc(field.location), field.value);
      }
    }
    ...
    builder.create<omtalk::KlassEndOp>(loc(klass.location));
